...
fn distance_two_points_fp(
    x_1_fp: FixedType, y_1_fp: FixedType, x_2_fp: FixedType, y_2_fp: FixedType
) -> FixedType {
...
// Assumes -pi <= theta <= +pi
fn cosine_8th_fp(theta_fp: FixedType) -> FixedType {
    //
    ...
    // factorial values
    let two_fact_fp = Fixed::from_felt(2 * SCALE_FP);
    let four_fact_fp = Fixed::from_felt(24 * SCALE_FP);
    let six_fact_fp = Fixed::from_felt(720 * SCALE_FP);
    let eight_fact_fp = Fixed::from_felt(40320 * SCALE_FP);
    ...
    let one_fp = Fixed::from_felt(1 * SCALE_FP);
    let value_fp = one_fp
...
fn sin_approx_fp(theta_deg_fp: FixedType) -> FixedType {
    let zero_fp = Fixed::from_felt(0);
    let one_fp = Fixed::from_felt(1 * SCALE_FP);
    ...
    let root_fp = Fixed::fp_sqrt(diff_fp);

    if (theta_deg_fp.sign == false) {
        // If theta_0 >= 0, then sin_theta_0 >= 0
        root_fp
    // Fixed::fp_sqrt(one_fp - cos_theta_squared_fp)
    } else {
        // If theta_0 < 0, then sin_theta_0 < 0
        zero_fp - root_fp
    // zero_fp - Fixed::fp_sqrt(one_fp - cos_theta_squared_fp)
    }
}
