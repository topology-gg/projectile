...
fn test_distance_two_points_fp() {
    let x_1_fp = Fixed::from_felt(0 * SCALE_FP);
    let y_1_fp = Fixed::from_felt(0 * SCALE_FP);
    let x_2_fp = Fixed::from_felt(4 * SCALE_FP);
    let y_2_fp = Fixed::from_felt(3 * SCALE_FP);
    ...
    let x_1_fp = Fixed::from_felt(-40 * SCALE_FP);
    let y_1_fp = Fixed::from_felt(-90 * SCALE_FP);
    let x_2_fp = Fixed::from_felt(-10 * SCALE_FP);
    let y_2_fp = Fixed::from_felt(-50 * SCALE_FP);
    let r_fp = distance_two_points_fp(x_1_fp, y_1_fp, x_2_fp, y_2_fp, );
    assert(r_fp.into() == 50 * SCALE_FP, 'result 2 invalid');

    let x_1_fp = Fixed::from_felt(3 * SCALE_FP);
    let y_1_fp = Fixed::from_felt(3 * SCALE_FP);
    let x_2_fp = Fixed::from_felt(15 * SCALE_FP);
    let y_2_fp = Fixed::from_felt(8 * SCALE_FP);
    let r_fp = distance_two_points_fp(x_1_fp, y_1_fp, x_2_fp, y_2_fp, );
    assert(r_fp.into() == 13 * SCALE_FP, 'result 3 invalid');

    let x_1_fp = Fixed::from_felt(-9 * SCALE_FP);
    let y_1_fp = Fixed::from_felt(3 * SCALE_FP);
    let x_2_fp = Fixed::from_felt(3 * SCALE_FP);
    let y_2_fp = Fixed::from_felt(-2 * SCALE_FP);
    let r_fp = distance_two_points_fp(x_1_fp, y_1_fp, x_2_fp, y_2_fp, );
...
fn test_cosine_8th_fp() {
    ...
    // 60 deg
    let theta_deg_fp = Fixed::from_felt(60 * SCALE_FP);
    let theta_fp = Fixed::fp_to_radians(theta_deg_fp);
    let cos_theta_fp = cosine_8th_fp(theta_fp);
    ...
    // 120 deg
    let theta_deg_fp = Fixed::from_felt(120 * SCALE_FP);
    let theta_fp = Fixed::fp_to_radians(theta_deg_fp);
    let cos_theta_fp = cosine_8th_fp(theta_fp);
    ...
    // 180 deg
    let theta_deg_fp = Fixed::from_felt(180 * SCALE_FP);
    let theta_fp = Fixed::fp_to_radians(theta_deg_fp);
    let cos_theta_fp = cosine_8th_fp(theta_fp);
...
//     // 30 deg
//     let theta_deg_fp = Fixed::from_felt(30 * SCALE_FP);
//     let sin_theta_deg_fp = sin_approx_fp(theta_deg_fp);
...
//     // 150 deg
//     let theta_deg_fp = Fixed::from_felt(150 * SCALE_FP);
//     let sin_theta_deg_fp = sin_approx_fp(theta_deg_fp);
//     // Should be = 0.5, but not exact so test if 0.4999 < mag < 0.5001
